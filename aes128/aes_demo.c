...
#define GETU32(pt) (((u32)(pt)[0] << 24) ^ ((u32)(pt)[1] << 16) ^((u32)(pt)[2] <<  8) ^ ((u32)(pt)[3])) /*u8->u32*/
...
/*加密函数，已测试*/
static void aes_encrypt(unsigned char pt[16],unsigned char ct[16],const u8 cipherKey[],size_t keyLen)
{
  u32 *rk;
  int Nr=10,i;
  u32 temp;
...
/*解密函数*/
static void aes_decrypt(unsigned char ct[16],  unsigned char pt[16], const u8 cipherKey[], size_t keyLen)
{
    u32 *rk;
...
void test_strcut(void)
{
	aeclist pdata;
	int len =sizeof(aeclist);
	memset(&pdata,0,len); 
	pdata.a =100;
	pdata.b = 1000;
...
int main(int argc, char *argv[])
{
      ...
      char p_text[25] = "hellotsdfaadialkfdaestae";
      char c_text[32];
      memset(c_text,0,32);
